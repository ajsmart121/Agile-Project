	...
	for($i = 0; $i < $questionCount; $i++){
		...
		if($QuestionsFindResult[$i][4]=="radiobuttons"){
			...
			for($j = 0; $j < $QuestionsFindResult[$i][3]; $j++){
				?>
				<input type="radio" id="answer[<?php $j+1 ?>]" name="answer[<?php $i+1 ?>]" value="<?php echo $AnswersFindResult[$j][1] ?>">
				<label for="answer[<?php $j+1 ?>]"><?php echo $AnswersFindResult[$j][1] ?></label><br>
				<?php
			}	
		}
		else{	
